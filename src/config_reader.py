...
class ConfigReader(object):
	...
	def readConf(self):
		...
		try:
			...
			logging.error("Read config_file error.\n%s" % str(error))
			return None
		...
		for option in self.option_list:
			if option not in item_dict:
				logging.error("Config file is lack of option: %s " % str(option))
				return None
        

        #检查种子文件是否存在
		if not os.path.isfile(item_dict["url_list_file"]):
			logging.error("Cant find the url_list_file in %s" % str(item_dict["url_list_file"]))
			return None

		#检查输出目录是否有效
		if not os.path.exists(item_dict["output_directory"]):
			logging.error("The output_directory %s is not exits!" % str(item_dict["output_directory"]))
			return None
		...
		try:
			...
			if item_dict["max_depth"] < 1 or item_dict["crawl_interval"] < 1 or \
				...
				logging.error("The items of the config are illegal." )
				return None
		except ValueError as error:
			logging.error("Some items of the config are illegal. \n%s" % str(error))
		...
		try:
			...
			logging.error('The target url of the config is illegal. \n%s' % str(error))
			return None
